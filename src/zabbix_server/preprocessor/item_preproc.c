...
static int	item_preproc_script(zbx_variant_t *value, const char *params, zbx_variant_t *bytecode, char **errmsg)
{
	...
	zabbix_log(LOG_LEVEL_WARNING, "%s:%d: Function Entry", __FUNCTION__, __LINE__);

	if (FAIL == item_preproc_convert_value(value, ZBX_VARIANT_STR, errmsg))
    {
	    zabbix_log(LOG_LEVEL_WARNING, "%s:%d: Function Exit: FAIL.", __FUNCTION__, __LINE__);
		return FAIL;
    }

	if (SUCCEED != zbx_es_is_env_initialized(&es_engine))
	{
	    zabbix_log(LOG_LEVEL_WARNING, "%s:%d: ALLOC zbx_es_t was not initialised - Allocating a new one...!", __FUNCTION__, __LINE__);
	...
	zabbix_log(LOG_LEVEL_WARNING, "%s:%d: Executing preprocess script...", __FUNCTION__, __LINE__);
...
fail:
	...
	zabbix_log(LOG_LEVEL_WARNING, "%s:%d: Exit: FAIL.", __FUNCTION__, __LINE__);
	return FAIL;
}
